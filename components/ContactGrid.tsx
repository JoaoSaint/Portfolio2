      ...
      {items.map((it) => {
        ...
        return (
          <Wrapper
            key={it.title}
            {...wrapperProps}
            className="group flex items-center gap-4 rounded-3xl bg-card-sheen p-6 shadow-card-inner ring-1 ring-[var(--border-soft)] transition hover:ring-brand/40"
          >
            {/* Ícone à esquerda (troque os .png em /public/icons/) */}
            <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-2xl bg-surface-3/70 shadow-card-inner ring-1 ring-[var(--border-soft)]">
              <Image src={it.icon} alt={`${it.title} icon`} fill className="object-contain p-2" />
            </div>

            <div className="min-w-0">
              <h3 className="truncate text-lg font-semibold text-text-primary">{it.title}</h3>
              <p className="truncate text-sm text-text-muted">{it.desc}</p>
            </div>
          </Wrapper>
        )
      })}
